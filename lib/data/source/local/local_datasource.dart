...
import 'package:flutter_pokemon_app/data/source/local/models/pokemon.dart';
import 'package:flutter_pokemon_app/data/source/local/models/pokemon_gender.dart';
import 'package:flutter_pokemon_app/data/source/local/models/pokemon_stats.dart';
...
class LocalDataSource {
  static Future<void> initialize() async {
    ...
    await Hive.openBox<PokemonHiveModel>(PokemonHiveModel.favBoxKey);
  ...
  Future<List<PokemonHiveModel>> getAllPokemons() async {
    ...
    final pokemons = List.generate(pokemonBox.length, (index) => pokemonBox.getAt(index)).whereType<PokemonHiveModel>().toList();
  ...
  Future<List<PokemonHiveModel>> getPokemons({required int page, required int limit}) async {
    ...
    final pokemons = List.generate(newPokemonCount, (index) => pokemonBox.getAt(start + index)).whereType<PokemonHiveModel>().toList();
  ...
  Future<void> saveFavoritePokemon(PokemonHiveModel pokemon) async {
    final pokemonBox = Hive.box<PokemonHiveModel>(PokemonHiveModel.favBoxKey);

    await pokemonBox.clear();
    await pokemonBox.put(PokemonHiveModel.favBoxKey, pokemon);
  }

  Future<List<PokemonHiveModel>> getAllFavoritePokemons() async {
    final pokemonBox = Hive.box<PokemonHiveModel>(PokemonHiveModel.favBoxKey);
