...
class PokemonBloc extends Bloc<PokemonEvent, PokemonState> {
  ...
  PokemonBloc(this._pokemonRepository) : super(PokemonState.initial()) {
    on<PokemonLoadStarted>(_onLoadStarted, transformer: (events, mapper) => events.switchMap(mapper));
    on<PokemonLoadMoreStarted>(_onLoadMoreStarted, transformer: (events, mapper) => events.switchMap(mapper));
  ...
  void _onLoadStarted(PokemonLoadStarted event, Emitter<PokemonState> emit) async {
    try {
      ...
      final pokemons = event.loadAll ? await _pokemonRepository.getAllPokemons() : await _pokemonRepository.getPokemons(page: 1, limit: pokemonsPerPage);
